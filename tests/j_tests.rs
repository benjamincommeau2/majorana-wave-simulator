...
fn applying_j_uses_expected_component_order() { // Defines a regression test that fixes the convention J(a,b,c,d) = (b,-a,d,-c).
  ...
  assert_eq!(transformed, [2.0, -1.0, 4.0, -3.0]); // Requires J = I tensor iY, with iY acting on the adjacent component pairs [a,b] and [c,d].
